...
export async function renderAvnacDocumentPreviewDataUrl(
  ...
  try {
    ...
    // StaticCanvas is base class; cast to Canvas for helper functions that only use read/write methods
    const sc = staticCanvas as unknown as import('fabric').Canvas
    await normalizeCanvasImagesForExport(sc, mod)
    for (const o of staticCanvas.getObjects()) ensureAvnacLayerId(o)
    try {
      migrateLegacyImageBlurFilters(sc, mod)
    } catch {
      /* ignore */
    }

    const vectorDocs = loadVectorBoardDocs(persistId)
    refreshAllVectorBoardInstances(sc, mod, vectorDocs)

    try {
      await loadCanvasGoogleFontsAndRelayout(sc, mod)
    } catch {
      /* best-effort */
    }
