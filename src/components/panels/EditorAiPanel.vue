<template>
  <div
    v-if="open"
    data-avnac-chrome
    class="pointer-events-auto fixed z-40 flex w-[min(100vw-1.5rem,360px)] max-h-[min(92dvh,720px)] flex-col overflow-hidden rounded-3xl border border-black/[0.08] bg-white/95 backdrop-blur-md"
    :style="panelStyle"
    role="dialog"
  ...
  >
    ...
    <div class="flex flex-1 flex-col gap-4 overflow-y-auto p-4">
      ...
      <div class="flex flex-col gap-1.5">
        ...
        <textarea
          ...
          rows="4"
          maxlength="2000"
          class="resize-none rounded-xl border border-black/[0.12] bg-neutral-50 px-3 py-2.5 text-[13px] text-neutral-900 placeholder:text-neutral-400 focus:border-[var(--accent,#6366f1)] focus:outline-none"
        />
      ...
      <div class="flex gap-3">
        ...
        <div class="flex flex-1 flex-col gap-1.5">
          ...
          <div class="flex items-center gap-1">
            <button
              type="button"
              class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-black/[0.1] bg-white text-neutral-700 hover:bg-neutral-50"
              @click="sceneCount = Math.max(2, sceneCount - 1)"
            >
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="5" y1="12" x2="19" y2="12"/></svg>
            </button>
            <div class="flex-1 rounded-lg border border-black/[0.1] bg-white py-1.5 text-center text-[13px] font-medium text-neutral-900">{{ sceneCount }}</div>
            <button
              type="button"
              class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-black/[0.1] bg-white text-neutral-700 hover:bg-neutral-50"
        ...
        <div class="flex flex-1 flex-col gap-1.5">
          ...
          <select
            v-model="style"
            class="h-8 rounded-lg border border-black/[0.1] bg-white px-2 text-[13px] text-neutral-900 focus:outline-none"
          >
      ...
      <!-- Error -->
      <div v-if="error" class="rounded-xl bg-red-50 px-3 py-2.5 text-[12px] text-red-700">
        {{ error }}
      </div>
    ...
    <!-- Footer -->
    <div class="shrink-0 border-t border-black/[0.06] p-3">
      <button
        type="button"
        :disabled="loading || prompt.trim().length < 5"
        class="flex w-full items-center justify-center gap-2 rounded-xl px-4 py-2.5 text-[13px] font-medium text-white disabled:opacity-40 disabled:cursor-not-allowed"
        style="background:var(--accent,#6366f1)"
        @click="generate"
      >
